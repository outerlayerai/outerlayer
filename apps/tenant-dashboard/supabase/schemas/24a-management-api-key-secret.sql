-- =============================================================================
-- Management API Key Secret Store (private schema)
-- =============================================================================
...
--   public.management_api_key GRANTs ALL to authenticated, and a signed-in user
--   must never be able to read key hashes.
--
-- Crypto: key = 'olk_' + base64url(32 CSPRNG bytes); the stored digest is hex
--   HMAC-SHA256(rawKey, MANAGEMENT_API_KEY_PEPPER) computed in the app layer (Web
--   Crypto), same construction as @repo/api-key-service's hashApiKey.
--
-- Revocation: management_api_key.revoked_at is set (not row deletion), so a
--   revoked key's audit trail survives — verify_management_api_key gates on it.
--
-- Dependencies: 01a-private-authz.sql (private schema), 24-management-api-key.sql
...
CREATE TABLE IF NOT EXISTS private.management_api_key_secret (
    management_api_key_id UUID PRIMARY KEY REFERENCES public.management_api_key(id) ON DELETE CASCADE,
    key_digest TEXT NOT NULL,
    pepper_version BIGINT NOT NULL DEFAULT 1,
    created_at TIMESTAMPTZ DEFAULT now(),

    CONSTRAINT uc_management_api_key_secret_digest UNIQUE (key_digest)
);

COMMENT ON TABLE private.management_api_key_secret IS 'HMAC digests for public.management_api_key rows — private schema, no PostgREST surface, access only via DEFINER RPCs';
...
-- at request time — an management API key acts as its creator, never a cached
...
-- -----------------------------------------------------------------------------
-- touch_management_api_key_last_used — bearer-path bookkeeping
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION private.touch_management_api_key_last_used(p_management_api_key_id uuid)
 RETURNS void
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- -----------------------------------------------------------------------------
-- set_management_api_key_secret — mint a key's digest
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION private.set_management_api_key_secret(p_management_api_key_id uuid, p_key_digest text, p_pepper_version bigint)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.verify_management_api_key(p_key_digest text)
 RETURNS jsonb
 LANGUAGE sql
 SECURITY INVOKER
...
CREATE OR REPLACE FUNCTION public.touch_management_api_key_last_used(p_management_api_key_id uuid)
 RETURNS void
 LANGUAGE sql
 SECURITY INVOKER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.set_management_api_key_secret(p_management_api_key_id uuid, p_key_digest text, p_pepper_version bigint)
 RETURNS void
 LANGUAGE sql
 SECURITY INVOKER
 SET search_path TO 'public'
