-- =============================================================================
-- Drops the agent-insights data plane: the `agent_finding` and `agent_theme`
-- tables and the `agents.findings.read` app_permission enum value that gated
-- them.
--
-- Postgres has no `ALTER TYPE ... DROP VALUE`, so the enum is rebuilt: rename
-- the old type, create the new one, retype the two columns that use it, and
-- drop the old type. Every RLS policy and function bound to the type by OID
-- is captured BEFORE the rename (while `public.app_permission` still renders
-- as that name, which then resolves to the NEW type on replay), dropped, and
-- replayed afterwards. Function ACLs are captured and replayed alongside,
-- because DROP + CREATE loses the EXECUTE revokes that keep the SECURITY
-- DEFINER helpers off the PostgREST surface.
-- =============================================================================
...
WHERE permission = 'agents.findings.read';

DELETE FROM public.custom_role_permission
WHERE permission = 'agents.findings.read';

-- API keys carry their scopes as an enum ARRAY, so the retired value has to
...
SET permissions = (
    ...
    WHERE p::text <> 'agents.findings.read'
)
WHERE permissions && ARRAY['agents.findings.read']::public.app_permission[];

-- ---------------------------------------------------------------------------
-- 2. Drop the tables. CASCADE clears their policies and indexes in one step.
--    Dropping the tables is the point of this migration, not an accidental
--    data-loss risk.
-- ---------------------------------------------------------------------------
-- squawk-ignore ban-drop-table
DROP TABLE IF EXISTS public.agent_finding CASCADE;
-- squawk-ignore ban-drop-table
DROP TABLE IF EXISTS public.agent_theme CASCADE;

-- ---------------------------------------------------------------------------
-- 3. Capture every policy and function that binds `public.app_permission`.
...
-- ---------------------------------------------------------------------------
-- 4. Drop the dependents, rebuild the enum, retype its columns, replay.
-- ---------------------------------------------------------------------------
DO $rebuild$
DECLARE
    stmt TEXT;
