...
1. `AC-057-01` **Given** a connected repo and a PR with at least one verified session link, **When** a reviewer opens the PR, **Then** a single bot comment lists each linked session with topic labels when available, duration, cost, and every model the session used, plus a deep link to that session's existing dashboard detail page; the comment's aggregated metadata line (which replaced the old header when the comment became the evidence comment — see `082-evidence-comment.md`) rolls up linked-session count, total agent time, and total cost, labeled as sums over linked sessions.
...
4. `AC-057-04` **Given** a PR with candidate session links that are all still pending, **When** the bot comments, **Then** the comment states it is waiting for session evidence and upgrades in place to the session table when a link confirms; a PR with no candidate links at all gets no comment (amended when the comment became the evidence comment — the earlier "No agent sessions linked yet" empty state posted on every PR of a connected repo, human-only ones included; the waiting and no-comment behaviors carry their own criteria in `082-evidence-comment.md`).
