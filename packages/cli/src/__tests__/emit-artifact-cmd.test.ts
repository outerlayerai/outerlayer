...
describe("runEmitArtifact — direct upload", () => {
  // proves AC-084-01 — the wire payload carries the client-computed identity
  ...
  // proves AC-084-03 — in CI the payload self-identifies: ci true, the
  ...
  // proves AC-084-04 — a plain developer machine outside any session or CI
  ...
  // proves AC-084-06 — with no anchor at all (no session, no CI PR context,
...
describe("runEmitArtifact — spool path (inside a recorded session)", () => {
  // proves AC-084-02 — inside a live recorded session (CLAUDECODE set and a
